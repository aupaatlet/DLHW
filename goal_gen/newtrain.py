...
from data.newdata2 import MultiTaskImageDataset
...
def init_trainer_config(configs):
    ...
    trainer_config['callbacks'] = [
        ...
        ModelCheckpoint(dirpath=ckpt_dir, save_top_k=1,every_n_epochs=5)
    ]
...
def experiment(variant):
    ...
    # dataset
    train_data = MultiTaskImageDataset(
        data_dir=variant["data_dir"],  # 从配置中读取路径
        resolution=variant.get("resolution", 128),
        resolution_before_crop=variant.get("resolution_before_crop", 160),
        center_crop=variant.get("center_crop", False),
        frame_offset=2,  # 固定偏移2帧
        is_training=True,
        color_aug=variant.get("color_aug", True),
    )
    
    val_data = MultiTaskImageDataset(
        data_dir=variant["data_dir"],  # 验证集使用相同目录
        resolution=variant.get("resolution", 128),
        resolution_before_crop=variant.get("resolution_before_crop", 160),
        center_crop=variant.get("center_crop", False),
        frame_offset=2,  # 固定偏移2帧
        is_training=False,  # 验证集不启用数据增强
        color_aug=False,
    )
